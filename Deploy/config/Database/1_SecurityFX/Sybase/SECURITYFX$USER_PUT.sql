IF EXISTS(SELECT * FROM sysobjects WHERE name = 'securityfx$user_put' AND type = 'P')
    DROP PROCEDURE securityfx$user_put
GO
...
BEGIN
    ...
    SELECT @PWD_TEXT = RTRIM(@PWD_TEXT)
    ...
    SELECT @PROFILES = RTRIM(@PROFILES)
    WHILE (LEN(@PROFILES) > 0) BEGIN
        -- Locate "<LF>" of next "ProfileID<LF>"
        SELECT @INDEX = CHARINDEX(CHAR(10), @PROFILES)
        IF (@INDEX = 0) BREAK

        -- Extract "ProfileCode" from list of profiles
        SELECT @PROFILE_CODE = CONVERT(INTEGER, LEFT(@PROFILES, @INDEX - 1))
        SELECT @PROFILES = SUBSTRING(@PROFILES, @INDEX + 1, LEN(@PROFILES))
    ...
    IF (@CREATED = 0) BEGIN
        ...
        SELECT @AUDIT_MESSAGE = 'Usuario fue modificado: ' + @USER_CODE
    END
    ELSE BEGIN
        SELECT @AUDIT_EVENT = 13
        SELECT @AUDIT_MESSAGE = 'Usuario fue creado: ' + @USER_CODE
    END
...
sp_procxmode securityfx$user_put, "anymode"
GO
