IF EXISTS(SELECT * FROM sysobjects WHERE name = 'securityfx$user_get' AND type = 'P')
    DROP PROCEDURE securityfx$user_get
GO
...
BEGIN
    ...
    IF (@@ROWCOUNT = 0) BEGIN
        RAISERROR 99999, 'Usuario no está definido: %s', @USER_CODE
        RETURN
    END
...
sp_procxmode securityfx$user_get, "anymode"
GO
