DROP PROCEDURE IF EXISTS DESKTOPFX$OBJECT_REN;
...
BEGIN
    ...
    IF (_LOGIN_ID = 0) THEN
        SET _MESSAGE_TEXT = 'No se puede renombrar un objeto global';
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = _MESSAGE_TEXT;
    END IF;
    ...
    IF EXISTS(SELECT *
        ...
        AND   NAME = _NEW_NAME)
    THEN
        SET _MESSAGE_TEXT = CONCAT('Nombre ya se encuentra en uso: ', _NEW_NAME);
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = _MESSAGE_TEXT;
    END IF;
